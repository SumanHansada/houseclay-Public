...
export const ContactedUsersView = ({ propertyID }: { propertyID: string }) => {
  ...
  const { contactUsers = [] } = currentProperty;

  const rows: SerializedUserRow[] = contactUsers.map((userInfo, index) => ({
    ...userInfo,
    _serial: index + 1,
  }));
  ...
  return (
    <LocalPaginatedTable
      tableTitle="Contacted Users"
      columns={columns}
      rows={rows}
      getRowId={(row) => row.phoneNo}
      noDataMessage="No contacted users found."
      isLoading={isLoading}
    />
  );
};
