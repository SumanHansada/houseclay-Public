...
export const ViewedUsersView = ({ propertyID }: { propertyID: string }) => {
  ...
  const { viewUsers = [] } = currentProperty;

  const rows: SerializedUserRow[] = viewUsers.map((userInfo, index) => ({
    ...userInfo,
    _serial: index + 1,
  }));
  ...
  return (
    <LocalPaginatedTable
      tableTitle="Viewed Users"
      columns={columns}
      rows={rows}
      getRowId={(row) => row.phoneNo}
      noDataMessage="No viewed users found."
      isLoading={isLoading}
    />
  );
};
