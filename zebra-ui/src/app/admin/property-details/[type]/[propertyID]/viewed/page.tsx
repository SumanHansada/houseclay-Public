...
export default function ViewedUsersPage() {
  ...
  const viewedUsers: User[] = dummyUserDataList;

  const totalRows = viewedUsers.length;
  ...
  const paginatedRows = useMemo(() => {
    ...
    return viewedUsers.slice(start, start + rowsPerPage);
  }, [viewedUsers, currentPage]);
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 flex flex-col bg-gray-100 py-8 px-16">
        <div className="bg-white shadow-sm rounded-xl p-5 flex flex-col gap-4 h-full">
          <h2 className="text-3xl">User Viewed</h2>
          <DataTable<User>
            columns={columns}
            data={paginatedRows}
